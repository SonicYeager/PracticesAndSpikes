...
static partial class Composer
{
    ...
    private static void Composing(List<string> identifiers, List<string> sidentifiers, List<string> toErease, List<string> mRC,
        ref string header, ref bool bDel, List<string> list)
    {
        for (var i = 0; i < list.Count; i++)
        {
            for (var i1 = 0; i1 < identifiers.Count; i1 += 2)
            {
                ...
                if (i + 1 < list.Count && header != "" && list[i + 1].Contains(sidentifiers[i1 == 0 ? 0 : 1]) &&
                    !(list[i + 1].Contains("dialog/IDD") || list[i + 1].Contains("dialog/CG_IDD") || list[i + 1].Contains("menu/IDR_") ||
                      list[i + 1].Contains("menu/IDC_MENU") || list[i + 1].Contains("menu/IDR_MENU") ||
                      list[i + 1].Contains("menu/POPUP_")))
                {
                    if (bDel)
                    {
    ...
    private static void FixingComposerLines(List<string> rc, List<string> toErease, List<string> mRC)
    {
        ...
        {
            if (rc[i].Contains("stringtable/") && rc[i].Contains('"') && !(rc[i].Contains("AFX_") || rc[i].Contains("IDS_") ||
                                                                           rc[i].Contains("IDC_") || rc[i].Contains("ID_") ||
                                                                           rc[i].Contains("IDR_")))
            {
