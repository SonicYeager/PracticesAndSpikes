...
namespace ResourceCompare
{
    ...
    {
        public static void DoComposerStandart(List<string> rcA, List<string> rcB)
        {
            List<string> identifiers = new List<string>() { "dialog/IDD", "dialog/CG_IDD", "menu/IDR_", "menu/POPUP_" };
            List<string> sidentifiers = new List<string>() { "dialog/", "menu/" };
            List<string> toErease = new List<string>();
            List<string> mRC = new List<string>();

            string header = "";
            bool bDel = false;

            Composing(identifiers, sidentifiers, toErease, mRC, ref header, ref bDel, rcA);
            foreach (string str in mRC)
            {
                if (str.Contains("DELETEME/"))
                    toErease.Add(str);
                rcA.Add(str);
            }
            foreach (string str in toErease)
            {
                rcA.Remove(str);
            }
            mRC.Clear();
        ...
        public static void DoFormatSpecifierFixLines(List<string> rcA, List<string> rcB)
        {
            ...
            {
                if (rcA[i].Contains("stringtable/") && rcA[i].Contains("%") && !(rcA[i].Contains("AFX_") || rcA[i].Contains("IDS_") || rcA[i].Contains("IDC_") || rcA[i].Contains("ID_") || rcA[i].Contains("IDR_")))
                {
            ...
            {
                if (rcB[i].Contains("stringtable/") && rcB[i].Contains("%") && !(rcB[i].Contains("AFX_") || rcB[i].Contains("IDS_") || rcB[i].Contains("IDC_") || rcB[i].Contains("ID_") || rcB[i].Contains("IDR_")))
                {
        ...
        {
            for (int i = 0; i < fSRCA.Count; i++)
            {
                rcA[i] += "| " + fSRCA[i];
            }

            for (int i = 0; i < fSRCB.Count; i++)
            {
                rcB[i] += "| " + fSRCB[i];
            }
        }
        ...
        public static List<string> MergeMenusToSection(List<List<string>> SortedSection, List<string> OriginalSection)
        {
            ...
            int startIndex = 0;
            for (int i = 0; i < SortedSection.Count; i++)
            {
        ...
        public static List<string> MergeDialogsToSection(List<List<string>> SortedSection, List<string> OriginalSection)
        {
            ...
            startIndex = OriginalSection.FindIndex(x =>  (x.Contains("IDD_") || x.Contains("CG_IDD_")) && (x.Contains("DIALOGEX") || x.Contains("DIALOG")));
            OriginalSection.RemoveRange(startIndex, OriginalSection.Count - startIndex);
            OriginalSection.AddRange(MergedSortedSection);
            //OriginalSection.Add("/////////////////////////////////////////////////////////////////////////////");
        ...
        public static List<string> BuildNewRC(List<string> StringTable, List<string> Dialoge, List<string> Menus, List<string> OriginalRC, List<string> sectionOrder)
        {
            int startIndex;
            int endIndex;

            startIndex = OriginalRC.FindIndex(x => x == "// String Table");
            endIndex = OriginalRC.FindIndex(startIndex, x => x == "/////////////////////////////////////////////////////////////////////////////");
            ...
            foreach (string header in sectionOrder)
            {
                if (OriginalRC.FindIndex(x => x == "// DESIGNINFO") == -1)
                {
                    if (sectionOrder.IndexOf("// Menu") < sectionOrder.IndexOf("// Dialog"))
                    {
                        ...
                        {
                            OriginalRC.InsertRange(OriginalRC.FindIndex(x => x == "// Generated from the TEXTINCLUDE 3 resource.") - 4, StringTable);
                        }
                        if (header == "// Menu")
                        {
                            OriginalRC.InsertRange(OriginalRC.FindIndex(OriginalRC.FindIndex(x => x == "// TEXTINCLUDE"), x => x == "/////////////////////////////////////////////////////////////////////////////") + 1, Menus);
                        }
                        if (header == "// Dialog")
                        {
                            OriginalRC.InsertRange(OriginalRC.FindIndex(OriginalRC.FindIndex(x => x == "// Menu"), x => x == "/////////////////////////////////////////////////////////////////////////////") + 1, Dialoge);
                        }
                    }
                    else
                    {
                        ...
                        if (header == "// Dialog")
                        {
                            OriginalRC.InsertRange(OriginalRC.FindIndex(OriginalRC.FindIndex(x => x == "// TEXTINCLUDE"), x => x == "/////////////////////////////////////////////////////////////////////////////") + 1, Dialoge);
                        }
                        if (header == "// Menu")
                        {
                            OriginalRC.InsertRange(OriginalRC.FindIndex(OriginalRC.FindIndex(x => x == "// Dialog"), x => x == "/////////////////////////////////////////////////////////////////////////////") + 1, Menus);
                        }
                    }
                }
                else
                {
                    if (sectionOrder.IndexOf("// Menu") < sectionOrder.IndexOf("// Dialog"))
                    {
                        if (header == "// String Table")
                        {
                            OriginalRC.InsertRange(OriginalRC.FindIndex(OriginalRC.FindIndex(x => x == "// DESIGNINFO"), x => x == "/////////////////////////////////////////////////////////////////////////////") + 1, StringTable);
                        }
                        if (header == "// Menu")
                        {
                            OriginalRC.InsertRange(OriginalRC.FindIndex(OriginalRC.FindIndex(x => x == "// TEXTINCLUDE"), x => x == "/////////////////////////////////////////////////////////////////////////////") + 1, Menus);
