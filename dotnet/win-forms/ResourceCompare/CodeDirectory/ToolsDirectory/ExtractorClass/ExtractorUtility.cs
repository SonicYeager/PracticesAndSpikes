...
static partial class Extractor
{
    ...
    private static void ExtractorStringTable(string[] identifiers, List<string> list, int i)
    {
    ...
    private static void ExtractorDialog(string[] identifiers, List<string> list, int i)
    {
    ...
    private static void ExtractorMenu(string[] identifiers, List<string> list, int i)
    {
    ...
    private static void RemoveBackChars(List<string> list, int i)
    {
        ...
        if (list[i].Contains(',')) list[i] = Cut(list[i].IndexOf(','), list[i].Length - list[i].IndexOf(','), list[i]);
        ...
        foreach (var line in list)
        {
            ...
            foreach (var chr in line)
            {
                if (Convert.ToString(chr).Contains('%'))
                    bTake = true;
                if (bTake && (Convert.ToString(chr).Contains('"') || Convert.ToString(chr).Contains('\t') ||
                              Convert.ToString(chr).Contains(']') || Convert.ToString(chr).Contains(' ') ||
                              Convert.ToString(chr).Contains('H') || Convert.ToString(chr).Contains('M') ||
                              Convert.ToString(chr).Contains('S') || Convert.ToString(chr).Contains('Y') ||
                              Convert.ToString(chr).Contains('|') || Convert.ToString(chr).Contains(')') ||
                              Convert.ToString(chr).Contains('-') || Convert.ToString(chr).Contains('\\')))
                {
                    bTake = false;
